configuration vTrainingLabUserThumbnails {
    ...
    foreach ($user in $Users) {
        ...
        if ($PSBoundParameters.ContainsKey('Filename')) {
            $userThumbnailPhotoPath = '{0}.{1}' -f (Join-Path -Path $ThumbnailPhotoPath -ChildPath $Filename), $Extension;
        }
        else {
            $userThumbnailPhotoPath = '{0}.{1}' -f (Join-Path -Path $ThumbnailPhotoPath -ChildPath $user.SamAccountName), $Extension;
        }

        vADUserThumbnailPhoto "$($user.SamAccountName)_Thumbnail" {
            DomainName = $DomainName;
            UserName = $user.SamAccountName;
            ThumbnailPhoto = $userThumbnailPhotoPath;
        }
